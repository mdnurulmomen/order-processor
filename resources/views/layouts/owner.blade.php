...
<head>
  ...
  @if (Auth::guard('owner')->check()) 
    <meta name="owner-id" content="{{ Auth::guard('owner')->user()->id }}" />
  @endif
  
  <title>Qupaid | Owner</title>
          ...
          <ul class="navbar-nav ml-auto mt-1">
            ...
            <li class="nav-item dropdown user-menu float-right">
              <a href="#" class="nav-link" data-toggle="dropdown" aria-expanded="false">
                ...
                <img src="{{ asset(\Auth::guard('owner')->user()->profile_picture ?? 'designToBeChangedHere') }}" class="user-image profile-user-img img-fluid" alt="User Image">
                
              </a>
              <ul class="ml-auto dropdown-menu">
                <!-- User image -->
                <li class="user-header bg-success">
                  <img src="{{ asset(\Auth::guard('owner')->user()->profile_picture ?? 'designToBeChangedHere') }}" class="img-circle" alt="User Image">

                  <p>
                    {{ \Auth::guard('owner')->user()->user_name }}
                ...
                <li class="user-footer row">
                  ...
                  <div class="col-4">
                    <a class="btn btn-default btn-flat"
                       onclick="document.getElementById('owner-logout-form').submit();">
                        {{ __('Logout') }}
                    </a>
                    <form id="owner-logout-form" action="{{ route('owner.logout') }}" method="POST" style="display: none;">
                        @csrf
                    </form>
                  </div>
                </li>
