								...
								<table class="table table-hover table-bordered table-striped text-center">
									...
									<tbody>
									  	...
									  	>
								    		...
								    		<td>{{ myKitchen.merchant!=null ? myKitchen.merchant.name : 'NA' }}</td>
								    		<td>
								    			<toggle-button 
		                                  			:sync="true" 
...
<script type="text/javascript">
	...
	export default {
		...
		created(){
			this.fetchMerchantKitchen();
		},

		methods : {
			fetchMerchantKitchen(){
				this.loading = true;
				axios
					.get('/api/my-kitchen/' + this.merchant_id)
					.then(response => {
						if (response.status == 200) {
							this.loading = false;
							this.myKitchen = response.data.kitchen || {};
							// console.log(response.data.kitchen);
							// console.log(this.myKitchen.merchant.name);
						}
					})
					.catch(error => {
						console.log(error);
