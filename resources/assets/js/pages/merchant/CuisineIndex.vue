								...
								<table class="table table-hover table-bordered table-striped text-center">
									...
									<tbody>
									  	<tr v-show="cuisinesToShow.length"
									    	v-for="(merchantCuisine, index) in cuisinesToShow"
									    	:key="merchantCuisine.id"
									  	>
									    	<td scope="row">{{ index + 1 }}</td>
								    		<td>{{ merchantCuisine.cuisine.name}}</td>
									  	</tr>
									  	<tr v-show="!cuisinesToShow.length">
								    		<td colspan="6">
	...
	export default {
		...
		methods : {
			...
			fetchMerchantAllMeals(){
				this.loading = true;
				axios
					.get('/merchant-cuisines/' + this.merchant_id + '/' + this.perPage +'?page='+ this.pagination.current_page)
					.then(response => {
		    ...
		    searchData() {
				...
				axios
				.get(
					"/merchant-cuisines/" + this.merchant_id + '/' + this.query +"/" + this.perPage +
				    "?page=" +
